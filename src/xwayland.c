		...
		struct wlr_xwayland_surface *xsurface, bool mapped)
{
	...
	view_init(view);
	wl_signal_init(&xwayland_view->events.set_icon);
	...
	CONNECT_SIGNAL(xwayland_view, xwayland_view, set_icon);
...
request_icon(struct xwayland_view *xwayland_view)
{
	...
	if (reply->type == XCB_ATOM_NONE) {
		wl_signal_emit_mutable(&xwayland_view->events.set_icon, NULL);
		goto no_icon;
	}
	...
	wl_signal_emit_mutable(&xwayland_view->events.set_icon, &icon);
no_icon:
	free(reply);
}
