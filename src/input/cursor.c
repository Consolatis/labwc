		...
		enum wlr_button_state button_state, uint32_t time_msec)
{
	...
	if (ctx.surface && !consumed_by_frame_context) {
		...
		wlr_seat_pointer_notify_button(seat->seat, time_msec, button, button_state);
	}
}
...
cursor_emulate_move_absolute(struct seat *seat, double x, double y, uint32_t time_msec)
{
	...
	if (!dx && !dy) {
		wlr_log(WLR_INFO, "dropping useless cursor_emulate: %.10f,%.10f", dx, dy);
		return;
	}
	...
	wlr_cursor_move(seat->cursor, NULL, dx, dy);
	process_cursor_motion(seat->server, time_msec);
}
